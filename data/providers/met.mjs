...
export function normalizeMetArtwork(record) {
  ...
  return compactObject({
    ...
    date: compactObject({
      display: record.objectDate,
      start: record.objectBeginDate,
      end: record.objectEndDate,
    }),
    ...
    images: preferred
      ? {
          preferred,
          alternates: uniqueStrings(record.additionalImages).map((url) => ({
            kind: "alternate",
            url,
          })),
        }
...
export function isMetPainting(record) {
  return (
...
export const metProvider = {
  ...
  capabilities: ["browse", "search", "by-id", "public-domain-filter"],
  async getArtworks(
    ...
    if (!objectIds.length) {
      ...
      objectIds = (search.objectIDs || []).slice(
        0,
    ...
    const records = await Promise.all(
      objectIds
        .slice(0, 20)
        .map((id) => fetchJson(`${apiBase}/objects/${id}`, context)),
    );
    const items = records
      .filter(isMetPainting)
