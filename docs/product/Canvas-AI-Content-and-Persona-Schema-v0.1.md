...
1. **作品内容增强**：对合规馆藏批量生成简介、观察提示、推荐问题、标签、关联、色板和无障碍描述，并分别保存事实、解释、推测与来源。
...
### 4.3 观察提示 `ObservationPrompt`

```ts
type ObservationPrompt = {
  id: string;
  prompt: string;                   // 一个动作，一次只引导观察一件事
  focus: "color" | "line" | "shape" | "space" | "light" | "texture" | "gesture" | "material";
  visualEvidenceIds: string[];
  claimIds: string[];
  difficulty: "open" | "focused" | "close_looking";
  answerRequired: false;
};
```

- 每件作品建议 2–4 条，避免测试口吻和暗示唯一正确答案。
- 必须能从可展示图像验证；低分辨率、遮挡或不确定细节不得成为提示。
- 与人格开场分离：现有 Prompt 的“不主动布置观察任务”继续有效。

### 4.4 推荐问题 `RecommendedQuestion`

```ts
type RecommendedQuestion = {
  id: string;
  text: string;
  intent: "artwork_fact" | "artist_context" | "making" | "visual_interpretation" | "comparison";
  answerableByClaimIds: string[];
  target: "catalog" | "persona";
  risk: "low" | "medium";
};
```

- 每个问题必须能由已入库主张回答；不得生成诱导模型猜测私密心理、医学诊断或未知动机的问题。
- `persona` 问题还必须通过对应人格的知识范围检查；否则只进入 `catalog` 问答。
...
type ArtworkTag = {
  id: string;
  namespace: "subject" | "genre" | "style" | "period" | "place" | "material" | "technique" | "visual";
  label: string;
...
type ArtworkRelation = {
  ...
  types: ("same_artist" | "same_series" | "same_subject" | "same_place" | "same_period" | "shared_material" | "visual_similarity")[];
  reason: string;
  claimIds: string[];
...
```ts
type SectionName =
  | "introduction" | "observationPrompts" | "recommendedQuestions"
  | "tags" | "relations" | "palette" | "accessibility"
  | "facts" | "interpretations" | "speculations";
...
type EnrichmentJob = {
  ...
  model: { provider: string; name: string; revision?: string; parametersHash: string };
  locale: string;
  artworkIds: string[];
...
2. 执行资格检查；`metadata_only` 不运行依赖像素的观察提示、色板和视觉无障碍描述。
3. 抽取原子主张并做来源绑定、冲突和重复检查。
4. 确定性生成/计算标签候选、关系候选和色板。
5. 生成简介、观察提示、问题与无障碍描述；输出必须通过结构化 Schema。
...
- 没有可展示图片时发布基础资料与来源，隐藏色板、视觉提示和视觉描述。
...
type EvidencePolicy = {
  ...
  speculationMode: "qualified_only";
  allowMedicalDiagnosis: false;
  allowPosthumousKnowledge: false;
};
```

人格差异必须落在可测量的语言实现上，不能只写“敏感、热情”等标签。`strength: 3` 仍不能越过证据和简洁性规则。
...
type TimelineEvent = {
  eventId: string;
  date: { start?: string; end?: string; display: string; precision: "day" | "month" | "year" | "range" | "unknown" };
  placeId?: string;
  claimIds: string[];
};
...
type DialogueResult = {
  ...
  segments: {
    text: string;
    layer: "fact" | "interpretation" | "persona_expression" | "uncertainty";
    claimIds: string[];
  }[];
  evidence: SourceRef[];
  disclosureId: string;
...
前端可以只展示自然回答，但服务端必须保留分段证据结构。`persona_expression` 是受声线规则约束的数字化身表达，不可被后续轮次当作历史事实检索。
...
- 从有限书信归纳的人格特征，表述为“材料显示/这套数字化身据此采用……语气”，不能宣称是完整人格真相。
